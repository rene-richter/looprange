...
TEST_CASE("linear spaced complex<double> values", "[linspace]") 
{
	...
	{
		for (auto x : loop::linspace(0., C(2.,1.), 4)) v.push_back(x);
		REQUIRE(v.size() == 5u);
		
		auto expected = Vec({ C(0.,0.), C(.5,0.25), C(1.,.5), C(1.5,.75), C(2.,1.) });
		REQUIRE(v == expected);
	}
}
...
TEST_CASE("linear spaced complex<float> values", "[linspace]") 
{
	...
	{
		for (auto x : loop::linspace(0.f, C(2.f,1.f), 4)) v.push_back(x);
		REQUIRE(v.size() == 5u);

		auto expected = Vec({ C(0.f,0.f), C(.5f,0.25f), C(1.f,.5f), C(1.5f,.75f), C(2.f,1.f) });
		REQUIRE(v == expected);
	}
}
